...
PUBLIC_SYNTHETIC_SOURCE_ID = "synthetic-authored-project09"
PUBLIC_SYNTHETIC_SOURCE_VERSION = "scaffold-2026-08-18"
...
class SourceManifest(BaseModel):
    ...
    def validate_rights_gate(self) -> SourceManifest:
        if self.source_type in {"external_source", "licence_cleared_external"}:
            if self.url is None:
        ...
        if self.public_release == "permitted_after_review" and self.source_type in {
            "external_source",
            "licence_cleared_external",
        }:
            if self.licence_decision != "licence_cleared":
...
class ModelRequest(BaseModel):
    ...
    input_provenance: Literal["unverified", "independently_authored_synthetic"] = "unverified"
...
class ModelResponse(BaseModel):
    ...
    def validate_response_state_and_hash(self) -> ModelResponse:
        ...
        expected_hash = stable_hash(payload)
        if self.response_hash is not None and self.response_hash != expected_hash:
            raise ValueError("response_hash does not match response content")
...
class PublicDevelopmentRow(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_public_identity_and_content(self) -> PublicDevelopmentRow:
        expected_id = make_row_id(
            self.category,
            self.prompt,
            self.source_id,
...
class AggregateResult(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_availability(self) -> AggregateResult:
        if self.status == "not_run" and (self.metrics or self.uncertainty):
            raise ValueError("not_run aggregates cannot contain performance metrics")
        return self
...
class PublicBundle(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_release_evidence(self) -> PublicBundle:
        if any(aggregate.status == "available" for aggregate in self.aggregates):
            raise ValueError("available aggregates require verified release evidence")
        if any(
            row.source_id != PUBLIC_SYNTHETIC_SOURCE_ID
            or row.source_version != PUBLIC_SYNTHETIC_SOURCE_VERSION
            for row in self.development_rows
        ):
